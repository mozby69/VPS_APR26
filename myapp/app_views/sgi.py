...
def webcam_qr_code_scanner_sgi_09pm_06am(request):
    if request.method == 'POST':
        ...
        if decoded_objects:
            ...
            #FOR TIMEOUT
            yesterday = current_date - timedelta(days=1)
            if "05:00" <= prac_time <= "10:00" and temporay.objects.filter(timein_names__isnull=False,EmpCode_id=EmpCode,login_status="timein_09pm_06am",date=yesterday).exists():
...
def webcam_qr_code_scanner_sgi_06am_03pm(request):
    if request.method == 'POST':
        ...
        if decoded_objects:
            ...
            if "13:00" <= prac_time <= "22:59" and temporay.objects.filter(EmpCode_id=EmpCode, timein_names__isnull=False, timeout_names__isnull=True,date=current_time.date()).exists():
                existing_entry3 = temporay.objects.filter(EmpCode_id=EmpCode, date=current_time.date()).first()
...
def webcam_qr_code_scanner_sgi_730am_430pm(request):
    if request.method == 'POST':
        ...
        if decoded_objects:
            ...
            if "14:00" <= prac_time <= "22:59" and temporay.objects.filter(EmpCode_id=EmpCode, timein_names__isnull=False, timeout_names__isnull=True,date=current_time.date()).exists():
                existing_entry3 = temporay.objects.filter(EmpCode_id=EmpCode, date=current_time.date()).first()
...
def insertData_730am_430pm(employee_number, current_time,employee_instance,request):
    ...
    if total_lateness.total_seconds() > 0:
        ...
        timein_names=employee_number,
        timein_timestamps=current_time,
        login_status="timein_730am_430pm",
        )
