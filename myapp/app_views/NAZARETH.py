...
def fetch_messages_nazareth(request):
    ...
    filtered_messages = [
        ...
        or 'timeout_10am_07pm_already' in messages.tags or 'timein_11am_08pm' in message.tags or 'timeout_11am_08pm' in message.tags
        or 'timeout_11am_08pm_already' in message.tags
...
def webcam_qr_code_scanner_nazareth_8am_5pm(request):
    if request.method == 'POST':
        ...
        if decoded_objects:
            ...
            if "14:00" <= prac_time <= "21:00" and temporay.objects.filter(timein_names__isnull=False,EmpCode_id=EmpCode,date=current_time.date()).exists():
...
def webcam_qr_code_scanner_nazareth_10am_07pm(request):
    if request.method == 'POST':
        ...
        if decoded_objects:
            ...
            if "17:00" <= prac_time <= "24:59" and temporay.objects.filter(EmpCode_id=EmpCode, timein_names__isnull=False, timeout_names__isnull=True,date=current_time.date()).exists():
                existing_entry3 = temporay.objects.filter(EmpCode_id=EmpCode, date=current_time.date()).first()
            ...
            if "18:00" <= prac_time <= "24:59" and temporay.objects.filter(Empname=full_name,
                ...
                if current_time - existing_entry_breakin_timestamps >= timedelta(seconds=6):
                    messages.error(request, f'TIMEOUT ALREADYsdf!<br>{full_name}', extra_tags='timeout_10am_07pm_already')
                    return HttpResponseRedirect(request.path)
        ...
        if decoded_objects:
            ...
            if "05:00" <= prac_time <= "15:00": 
                ...
                if existing_entry is None: 
                    ...
                    messages.success(request, f'TIME IN SUCCESSFULLY!<br> {full_name}', extra_tags='timein_11am_08pm')
                    return HttpResponseRedirect(request.path)
            ...
            if "17:00" <= prac_time <= "24:59" and temporay.objects.filter(EmpCode_id=EmpCode, timein_names__isnull=False, timeout_names__isnull=True,date=current_time.date()).exists():
                ...
                if current_time - existing_entry_breakin_timestamps >= timedelta(seconds=5):
                    ...
                    messages.success(request, f'TIME OUT SUCCESSFULLY<br>{full_name}', extra_tags='timeout_11am_08pm')    
                    temporay.objects.filter(EmpCode_id=EmpCode, date=current_time.date()).update(timeout_names=EmpCode,timeout_timestamps=current_time)
                    return HttpResponseRedirect(request.path)
            ...
            if "18:00" <= prac_time <= "24:59" and temporay.objects.filter(Empname=full_name,
                ...
                if current_time - existing_entry_breakin_timestamps >= timedelta(seconds=6):
                    messages.error(request, f'TIMEOUT ALREADYsdf!<br>{full_name}', extra_tags='timeout_11am_08pm_already')
                    return HttpResponseRedirect(request.path)
